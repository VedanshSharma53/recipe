...
const generateRecipe = async (req, res) => {
  ...
  try {
      const response = await axios.post(
          "https://api.openai.com/v1/chat/completions",
          {
              ...
              messages: [
                  {
                      role: "system",
                      content: "You are a helpful chef that provides recipes based on given ingredients."
                  },
                  {
                      role: "user",
                      content: `I have ${ingredients.join(", ")}. What recipe can I make?`
                  }
              ],
              temperature: 0.7,
              max_tokens: 300
      ...
      const aiRecipe = response.data.choices[0].message.content;
